import json
import logging
from datetime import datetime
from pathlib import Path

from config import settings
from clients.llm import LLMClient
from clients.comfyui import ImageClient
from utils.chapter_split import split_chapters
from utils.json_repair import repair_json
from utils.log_helper import append_log
from prompts.script import build_script_prompt, build_sd_prompt

logger = logging.getLogger(__name__)


def _project_dir(project_id: str) -> Path:
    return settings.PROJECTS_DIR / project_id


def _update_status(project_id: str, **kwargs):
    path = _project_dir(project_id) / "status.json"
    status = {}
    if path.exists():
        status = json.loads(path.read_text())
    if "created_at" not in status:
        status["created_at"] = datetime.now().isoformat()
    status.update(kwargs)
    path.write_text(json.dumps(status, ensure_ascii=False, indent=2))


def _load_checkpoint(project: Path) -> set:
    cp_path = project / "checkpoint.json"
    if cp_path.exists():
        try:
            data = json.loads(cp_path.read_text())
            return set(data.get("completed_stages", []))
        except Exception:
            pass
    return set()


def _save_checkpoint(project: Path, completed: set, last_stage: str):
    cp_path = project / "checkpoint.json"
    cp_path.write_text(json.dumps({
        "completed_stages": sorted(completed),
        "last_stage": last_stage,
        "timestamp": datetime.now().isoformat(),
    }, ensure_ascii=False, indent=2))


async def run_pipeline(project_id: str):
    project = _project_dir(project_id)

    proj_settings = {}
    settings_path = project / "settings.json"
    if settings_path.exists():
        proj_settings = json.loads(settings_path.read_text())

    llm = LLMClient(
        proj_settings.get("llm_base_url") or settings.LLM_BASE_URL,
        proj_settings.get("llm_api_key") or settings.LLM_API_KEY,
        proj_settings.get("llm_model") or settings.LLM_MODEL,
    )

    image_provider = proj_settings.get("image_provider") or settings.IMAGE_PROVIDER
    img = ImageClient(
        base_url=proj_settings.get("image_base_url") or settings.IMAGE_BASE_URL,
        api_key=proj_settings.get("image_api_key") or settings.IMAGE_API_KEY,
        model=proj_settings.get("image_model") or settings.IMAGE_MODEL,
        provider=image_provider,
        jimeng_ak=proj_settings.get("jimeng_access_key") or settings.JIMENG_ACCESS_KEY_ID,
        jimeng_sk=proj_settings.get("jimeng_secret_key") or settings.JIMENG_SECRET_ACCESS_KEY,
    )

    completed = _load_checkpoint(project)

    try:
        _update_status(project_id, status="running", current_step="split_chapters", progress=0, error=None)
        append_log(project_id, "pipeline", "Pipeline started")

        # 1. Read novel and split chapters
        if "split_chapters" not in completed:
            novel_text = (project / "novel.txt").read_text(encoding="utf-8")
            chapters = split_chapters(novel_text)
            logger.info("Split into %d chapters", len(chapters))
            append_log(project_id, "split_chapters", f"Split into {len(chapters)} chapters")
            completed.add("split_chapters")
            _save_checkpoint(project, completed, "split_chapters")
        else:
            novel_text = (project / "novel.txt").read_text(encoding="utf-8")
            chapters = split_chapters(novel_text)
            append_log(project_id, "split_chapters", "Skipped (checkpoint)")

        # 2. Generate script for each chapter via LLM
        if "generate_script" not in completed:
            _update_status(project_id, current_step="generate_script", progress=10)
            append_log(project_id, "generate_script", "Generating script...")
            all_characters = {}
            all_scenes = {}
            all_props = {}
            all_episodes = []

            for ch in chapters:
                messages = build_script_prompt(ch["content"], ch["index"])
                raw = await llm.chat(messages, json_mode=True)
                episode = repair_json(raw)

                for c in episode.get("characters", []):
                    all_characters[c["id"]] = c
                for s in episode.get("scenes", []):
                    all_scenes[s["id"]] = s
                for p in episode.get("props", []):
                    all_props[p["id"]] = p

                all_episodes.append({
                    "episode": ch["index"],
                    "title": ch["title"],
                    "characters": episode.get("characters", []),
                    "scenes": episode.get("scenes", []),
                    "shots": episode.get("shots", []),
                })

                pct = 10 + int(40 * ch["index"] / len(chapters))
                _update_status(project_id, progress=pct)
                append_log(project_id, "generate_script", f"Chapter {ch['index']} done")

            script = {
                "episodes": all_episodes,
                "characters": list(all_characters.values()),
                "scenes": list(all_scenes.values()),
                "props": list(all_props.values()),
            }
            script_path = project / "script.json"
            script_path.write_text(json.dumps(script, ensure_ascii=False, indent=2))
            logger.info("Script saved: %s", script_path)
            append_log(project_id, "generate_script", f"Script saved ({len(all_episodes)} episodes)")
            completed.add("generate_script")
            _save_checkpoint(project, completed, "generate_script")
        else:
            append_log(project_id, "generate_script", "Skipped (checkpoint)")

        # Load script for subsequent stages
        script = json.loads((project / "script.json").read_text())
        all_characters = {c["id"]: c for c in script.get("characters", [])}
        all_scenes = {s["id"]: s for s in script.get("scenes", [])}
        all_props = {p["id"]: p for p in script.get("props", [])}
        all_episodes = script.get("episodes", [])

        # 3. Generate character reference images
        if "generate_characters" not in completed:
            _update_status(project_id, current_step="generate_characters", progress=50)
            append_log(project_id, "generate_characters", f"Generating {len(all_characters)} characters...")
            chars_dir = project / "assets" / "characters"
            for cid, char in all_characters.items():
                char_dir = chars_dir / cid
                char_dir.mkdir(parents=True, exist_ok=True)
                prompt = f"character portrait, {char['appearance']}, white background, full body, anime style"
                try:
                    await img.text2img(prompt, "lowres, blurry", char_dir / "ref.png")
                    append_log(project_id, "generate_characters", f"Character {char.get('name', cid)} done")
                except Exception as e:
                    logger.error("Failed to generate character %s: %s", cid, e)
                    append_log(project_id, "generate_characters", f"Failed: {char.get('name', cid)} - {e}", "error")
            completed.add("generate_characters")
            _save_checkpoint(project, completed, "generate_characters")
        else:
            append_log(project_id, "generate_characters", "Skipped (checkpoint)")

        # 4. Generate scene reference images
        if "generate_scenes" not in completed:
            _update_status(project_id, current_step="generate_scenes", progress=60)
            append_log(project_id, "generate_scenes", f"Generating {len(all_scenes)} scenes...")
            scenes_dir = project / "assets" / "scenes"
            for sid, scene in all_scenes.items():
                scene_dir = scenes_dir / sid
                scene_dir.mkdir(parents=True, exist_ok=True)
                prompt = f"background art, {scene['description']}, anime style, no characters"
                try:
                    await img.text2img(prompt, "lowres, blurry", scene_dir / "ref.png")
                    append_log(project_id, "generate_scenes", f"Scene {scene.get('name', sid)} done")
                except Exception as e:
                    logger.error("Failed to generate scene %s: %s", sid, e)
                    append_log(project_id, "generate_scenes", f"Failed: {scene.get('name', sid)} - {e}", "error")
            completed.add("generate_scenes")
            _save_checkpoint(project, completed, "generate_scenes")
        else:
            append_log(project_id, "generate_scenes", "Skipped (checkpoint)")

        # 4.5. Generate prop reference images
        if "generate_props" not in completed:
            _update_status(project_id, current_step="generate_props", progress=65)
            append_log(project_id, "generate_props", f"Generating {len(all_props)} props...")
            props_dir = project / "assets" / "props"
            for pid_key, prop in all_props.items():
                prop_dir = props_dir / pid_key
                prop_dir.mkdir(parents=True, exist_ok=True)
                prompt = f"item illustration, {prop['description']}, white background, detailed, anime style"
                try:
                    await img.text2img(prompt, "lowres, blurry", prop_dir / "ref.png")
                    append_log(project_id, "generate_props", f"Prop {prop.get('name', pid_key)} done")
                except Exception as e:
                    logger.error("Failed to generate prop %s: %s", pid_key, e)
                    append_log(project_id, "generate_props", f"Failed: {prop.get('name', pid_key)} - {e}", "error")
            completed.add("generate_props")
            _save_checkpoint(project, completed, "generate_props")
        else:
            append_log(project_id, "generate_props", "Skipped (checkpoint)")

        # 5. Generate frames for each shot
        if "generate_frames" not in completed:
            _update_status(project_id, current_step="generate_frames", progress=70)
            total_shots = sum(len(ep["shots"]) for ep in all_episodes)
            append_log(project_id, "generate_frames", f"Generating {total_shots} frames...")
            done_shots = 0

            visual_style = proj_settings.get("visual_style", "anime_cel")
            chars_dir = project / "assets" / "characters"

            for ep in all_episodes:
                ep_dir = project / "frames" / f"episode_{ep['episode']}"
                ep_dir.mkdir(parents=True, exist_ok=True)

                for shot in ep["shots"]:
                    shot_id = shot.get("shot_id", f"s{ep['episode']}_unknown")
                    positive, negative = build_sd_prompt(shot, all_characters, all_scenes, style=visual_style)

                    char_ids = shot.get("characters", [])
                    ref_path = None
                    if char_ids:
                        candidate = chars_dir / char_ids[0] / "ref.png"
                        if candidate.exists():
                            ref_path = candidate

                    output = ep_dir / f"{shot_id}.png"
                    try:
                        if ref_path:
                            await img.text2img_with_ref(positive, negative, ref_path, output)
                        else:
                            await img.text2img(positive, negative, output)
                    except Exception as e:
                        logger.error("Failed to generate shot %s: %s", shot_id, e)
                        append_log(project_id, "generate_frames", f"Failed: {shot_id} - {e}", "error")

                    done_shots += 1
                    pct = 70 + int(30 * done_shots / max(total_shots, 1))
                    _update_status(project_id, progress=pct)

            completed.add("generate_frames")
            _save_checkpoint(project, completed, "generate_frames")
            append_log(project_id, "generate_frames", f"All {total_shots} frames done")
        else:
            append_log(project_id, "generate_frames", "Skipped (checkpoint)")

        _update_status(project_id, status="completed", current_step="done", progress=100)
        append_log(project_id, "pipeline", "Pipeline completed")
        logger.info("Pipeline completed for project %s", project_id)

    except Exception as e:
        logger.exception("Pipeline failed for project %s", project_id)
        append_log(project_id, "pipeline", f"Pipeline failed: {e}", "error")
        _update_status(project_id, status="failed", error=str(e))
